 ...
 void Game::game()
{
	...
	srand(time(NULL));
	this->set_who_first(rand() % this->get_how_many_players());
	Player tempsz;
	tempsz = players[0];
	players[0] = players[this->get_who_first()];
	players[this->get_who_first()] = tempsz;
	do
	{
		...
		//do t¹d dobrze 
			...
			for (int i = 0; i < get_how_many_players() - 1; i++) {
				Player tempsz;
				tempsz = players[i];
	...
	for(int i=0;i<this->get_how_many_players();i++)
		{
			if(players[i].get_set()==this->get_how_many_set())
			{
				...
				cout << endl << "Chcesz zagrac ponownie?T/N" << endl;
				char temp;
				cin >> temp;
				if (temp == 'T') {
					system("cls");
